...
const patentData = [
  ...
  },
  {
    ...
    patentLink: "https://patents.google.com/patent/US8887512B2/en?q=A47J41%2f005",
  ...
  },
  {
    ...
    patentName: "Wearable device and method for stress detection, emotion recognition and emotion management",
  ...
  },
  {
    ...
    patentLink: "https://patents.google.com/patent/US10661433B2/en?q=G05D1%2f0251",
  ...
  },
  {
    ...
    abstract: "A robot for providing medical assistance, delivery medications and testing materials to a patient is disclosed. The robot includes a set of wheels and a mechanism for bringing the robot to a patient as well as a mechanism for avoiding objects and individuals. A computer and program provides initial guidance and instructions for medications and testing and medical history of a patient. The computer programs the robot to distribute medications, perform and record test results and in an emergency to telephone a healthcare provider. The robot also includes a transmitter and receiver for sending pictures, test results and medical history to a remote station. Further the robot includes a storage battery, a charger and means for connecting the charger to a source of electricity.",
    status: "abandoned",
    year: 2014,
    lowHallucinationExample: "",
    mediumHallucinationExample: "",
...
async function importPatents() {
  try {
    ...
    // Clear existing patents (optional)
    console.log("Clearing existing patents...");
    await Patent.deleteMany({});
    
    // Import new patents
    console.log("Importing patents...");
    const savedPatents = await Patent.insertMany(patentData);
    
    console.log(`Successfully imported ${savedPatents.length} patents:`);
    savedPatents.forEach((patent, index) => {
      console.log(`${index + 1}. ${patent.patentName} (${patent.category})`);
    });

    // Verify the import
    const totalPatents = await Patent.countDocuments();
    console.log(`\nTotal patents in database: ${totalPatents}`);
    
    // Count by category
    const smartDeviceCount = await Patent.countDocuments({ category: "Smart Interactive Beverage & Food Containers" });
    const healthcareCount = await Patent.countDocuments({ category: "Healthcare" });
    const automationCount = await Patent.countDocuments({ category: "Automation" });
    
    console.log(`Smart Interactive Beverage & Food Containers: ${smartDeviceCount}`);
    console.log(`Healthcare: ${healthcareCount}`);
    console.log(`Automation: ${automationCount}`);
    
    console.log("\nPatent import completed successfully!");
